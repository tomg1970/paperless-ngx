...
class TestFileHandling(DirectoriesMixin, FileSystemAssertsMixin, TestCase):
    ...
    def test_file_renaming(self):
        ...
        self.assertEqual(
            ...
            settings.ORIGINALS_DIR / Path(f"{document.pk:07d}.pdf"),
        )
        ...
        self.assertIsDir(
            os.path.join(settings.ORIGINALS_DIR, "test"),
        )
        self.assertIsNotDir(
            os.path.join(settings.ORIGINALS_DIR, "none"),
        )
        self.assertIsFile(
    ...
    def test_file_renaming_missing_permissions(self):
        ...
        self.assertEqual(
            document.source_path,
            settings.ORIGINALS_DIR / Path("none/none.pdf"),
        )
    ...
    def test_file_renaming_database_error(self):
        ...
        with mock.patch("documents.signals.handlers.Document.objects.filter") as m:
            ...
            # Check proper handling of files
            self.assertIsFile(document.source_path)
            self.assertIsFile(os.path.join(settings.ORIGINALS_DIR, "none/none.pdf"))
            self.assertEqual(document.filename, "none/none.pdf")
    ...
    def test_document_delete_trash(self):
        ...
        self.assertIsNotFile(
            os.path.join(settings.TRASH_DIR, "none/none.pdf"),
        )
        document.delete()
        self.assertIsNotFile(
            os.path.join(settings.ORIGINALS_DIR, "none/none.pdf"),
        ...
        self.assertIsNotFile(
            os.path.join(settings.TRASH_DIR, "none_01.pdf"),
        )
    ...
    def test_filename_relative(self):
        ...
        self.assertEqual(
            doc.source_path,
            settings.ORIGINALS_DIR / Path("etc/something/doc1.pdf"),
        )
    ...
    def test_nested_directory_cleanup(self):
        ...
        # Check proper handling of files
        self.assertIsDir(
            os.path.join(settings.ORIGINALS_DIR, "none/none"),
        )
...
class TestFileHandlingWithArchive(DirectoriesMixin, FileSystemAssertsMixin, TestCase):
    ...
    def test_create_with_format(self):
        ...
        self.assertEqual(
            doc.source_path,
            settings.ORIGINALS_DIR / Path("none/my_doc.pdf"),
        )
        self.assertEqual(
            doc.archive_path,
            settings.ARCHIVE_DIR / Path("none/my_doc.pdf"),
        )
